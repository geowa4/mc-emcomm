...
`mc_emcomm` is a single web application that runs the online presence and member-management workflow for Monroe County EmComm, the Monroe County, NY ARES/RACES organization (a 501(c)(3) nonprofit). It serves three audiences: the general public, who see an informational website; approved members, who use a private portal for training records, operations, equipment tracking, and net logging; and administrators, who manage membership and catalogs. It replaces the organization's existing Hugo static website at monroecountyemcomm.org and consolidates manual and spreadsheet-based processes into one system. The application is built with Phoenix LiveView and is hosted on Fly.io.
...
5. operations with multiple named geofenced locations and attendance;
...
| Operations list `/operations` | Yes (summary) | Yes | Yes |
| Operations detail incl. locations/attachments `/app/operations` | No | Yes | Yes |
...
Phoenix 1.8.x; Phoenix LiveView (the 1.1 line shipped mid-2025; the published line is now 1.2.x); OTP 25+ floor; Bandit. Contexts: Accounts (users, auth, `is_admin`); Members (profile, roles, QTH, quadrant, status, membership audit); Capabilities (catalog + `member_capabilities`); Courses (catalog + `member_courses`); Certifications (catalog + `member_certifications`); Assets (inventory, `public_id`, QR); Sightings (rows and lifecycle); Operations (operations, locations, attachments, attendance); Net (sessions, check-ins); Content (public pages, resources documents, calendar).
...
| `operation_location_id` | FK → operation_locations | nullable |
...
Indexes: `asset_id`; `member_id`; `operation_id`; `session_token`; `visited_at`; GiST `point`. Columns in the first three groups are admin-only in every query and view.
...
| `name` | string | not null; defaults to `"Primary Site"` when the operation is created with exactly one location |
| `point` | geography(Point,4326) | not null |
| `geofence_radius_m` | integer | not null, default `500` |
| `notes` | text | nullable |
| `position` | integer | not null, default `0` |

Indexes: `operation_id`; GiST `point`; unique `(operation_id, name)`.
...
Indexes: unique `(operation_id, member_id)`.
...
**`:public`** — `/`, `/about`, `/training`, `/resources` (list; downloads gated), `/calendar`, `/donations`, `/operations`, `/operations/:id` (summary), `/a/:public_id/s`.
**`:member`** (approved) — `/app`, `/app/profile`, `/app/operations`, `/app/operations/:id`, `/app/inventory`, `/app/inventory/:public_id`, `/app/net`, `/app/net/:id`.
**`:admin`** — `/admin`, `/admin/members`, `/admin/operations`, `/admin/inventory`, `/admin/capabilities`, `/admin/courses`, `/admin/certifications`, `/admin/documents`.
...
**Update point 2 — form submit.** `call_sign`, optional `note`, `claimed_responsibility`. The row is UPDATED with `member_id` (auto-linked if the call sign matches an approved member, or from the logged-in session), `submitted_at`, `verified`, and the geofence-matched `operation_id`/`operation_location_id`. If the submitter is an approved member and the sighting matched an active operation location, an `operation_attendance` row with `source = asset_checkin` MUST be created, carrying the `sighting_id`.
...
### Operations with multiple named locations & attachments
Admin CRUD. An operation holds many `operation_locations`, each with a name, a map-placed point, and `geofence_radius_m`. Creating an operation with a single location defaults its name to "Primary Site". The detail map renders every location as a marker plus a radius circle. Attachments upload to Tigris and each MUST have a `description`.
...
## 10. PostGIS design
PostGIS is used ONLY for storing points and for geofence matching. Matching MUST query `operation_locations` joined to active operations (the `starts_at`/`ends_at` window contains the sighting time) using `ST_DWithin(location.point, sighting.point, location.geofence_radius_m)` on geography (meters), ordered by `ST_Distance`, limit 1. All point columns are GiST-indexed. No quadrant is computed geometrically.

## 11. File storage
A private Fly Tigris bucket holds all uploads (asset images, course/certification evidence, operation attachments, documents). LiveView `allow_upload` MUST use `external:` with a presigned POST form from `ReqS3.presign_form/1`; viewing MUST use short-lived URLs from `ReqS3.presign_url/1`. Tests MUST stub the presign functions. Standard env vars `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `AWS_ENDPOINT_URL_S3` drive ReqS3.

## 12. Maps
Leaflet 1.9.4 is bundled via esbuild with OpenStreetMap tiles; map-click pin drop is used for QTH and operation-location placement. Leaflet 1.9.4 is the last stable 1.x (2.0 is alpha); pin exactly 1.9.4. The app MUST comply with the OSM Tile Usage Policy: identifying User-Agent, "© OpenStreetMap contributors" attribution, and never bulk-download or offline-cache tiles.
...
6. **Operations & geofence** — operations, multiple named locations (Primary Site default), attachments with required `description`, attendance, `ST_DWithin` matching, map circles. Benchmark: in-radius/in-window matches; outside/expired does not.
...
Confirm with leadership: the W2ARM (ex-N2MPE) callsign transition, the exact weekly net day/time, and the repeater PL/offset. Future work: ICS-form generation from net logs; operation after-action report export; optional self-hosted Nominatim; a static quadrant map overlay; FCC ULS call-sign lookup.
