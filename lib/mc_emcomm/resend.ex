defmodule McEmcomm.Resend do
  @moduledoc """
  Minimal Resend API client (Req). The API key comes from
  `config :mc_emcomm, :resend_api_key` (set in `config/runtime.exs`).

  In test, `config :mc_emcomm, McEmcomm.Resend, req_options: [plug: {Req.Test, McEmcomm.Resend}]`
  routes every request to a `Req.Test` stub. Consumers depend on the
  `McEmcomm.Resend.Client` behaviour instead and are tested against
  `McEmcomm.ResendMock` (Mox).
  """

  @behaviour McEmcomm.Resend.Client
  ...
  defp api_key do
    case Application.get_env(:mc_emcomm, :resend_api_key) do
  ...
  defp req_options do
    :mc_emcomm |> Application.get_env(__MODULE__, []) |> Keyword.get(:req_options, [])
  end
end
