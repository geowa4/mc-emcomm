...
5. operations with multiple named geofenced locations, attachments, RSVPs, and attendance;
...
| Operations portal incl. locations, attachments, RSVPs, attendance `/app/operations` | No | Yes | Yes |
| RSVP to an operation (until it ends) | No | Yes | Yes |
...
New registrations create the `users` row and a `pending` member profile in one transaction (§9.2). Only `approved` members pass the `:member` on_mount gate; admins pass it regardless of profile. When a new account registers, holders of positions flagged `notify_on_new_member` MUST be emailed (§27); confirmation sends nothing further.
...
- **Operations** — operations, named geofenced locations, attachments, RSVPs, attendance, PostGIS geofence matching.
...
### 7.28 `operation_rsvps`

A member's intent to attend, distinct from §7.15 attendance (who actually showed up, RSVP or not). `operation_id` FK (delete_all) not null; `member_id` FK → members (delete_all) not null; `response` enum `yes|maybe|no` not null; `note` (≤ 500 chars, trimmed, blank stored as null); `responded_at` utc_datetime_usec not null (refreshed on every change). Unique `(operation_id, member_id)`; index `member_id`. `Operations.rsvp/3` upserts with `on_conflict: {:replace, ...}`, so a member has exactly one row per operation and re-replying changes it; it refuses with `{:error, :operation_ended}` once `ends_at` has passed. `operation_id`, `member_id`, and `responded_at` are never cast. `list_rsvps/1` orders going, maybe, no, then by member name; `rsvp_counts/1` reports all three counts.
...
- **Login** (`/users/log-in`): a magic-link form (`#login_form_magic`) whose response is identical for known and unknown emails, and a password form (`#login_form_password`) with a remember-me checkbox that posts to the session controller. Every password input renders a client-side show/hide toggle (`#<input id>-toggle`, `aria-pressed`, `aria-controls`) that never round-trips to the server. Invalid credentials flash "Invalid email or password" without disclosing whether the address exists. In dev, when the mailer is `Swoosh.Adapters.Local`, `#local-mail-notice` links to `/dev/mailbox`. For an already-logged-in user the page enters re-authentication mode ("You need to reauthenticate"), hides Register, and prefills the email.
- **Confirmation** (`/users/log-in/:token`): an unconfirmed user sees "Confirm and stay logged in" (`#confirmation_form`); confirming sets `confirmed_at` and mints a session; the new-member notification (§27) was already sent at registration, so confirmation does not send another. A confirmed user sees the plain login form ("Keep me logged in on this device"). Tokens are single-use; reuse or an invalid token flashes "Magic link is invalid or it has expired." Login is required at confirmation as pre-stuffing defense and MUST NOT be weakened.
...
- **Account** (`/users/settings`, labelled "Account" in menus): change email (confirmation link to the new address, `#email_form`), change password (`#password_form`, 12–72 chars, rotates the session; an account that already has a password must supply it as `#user_current_password`, verified only on submit, while a magic-link-only account sets its first password without one), and a two-factor status line with a link to enrollment. The settings and two-factor pages require **sudo mode**: the on_mount hook accepts an authentication at most 10 minutes old (`Accounts.sudo_mode?/2` defaults to 20 minutes for controller assertions); otherwise "You must re-authenticate to access this page."
...
- **Admin** (`/admin/operations`, `/new`, `/:id/edit`, `/:id/copy`): the index lists operations with real edit links; creating redirects to the edit page ("Operation created"). `#operation-form`: title, description, starts/ends, visibility. `#location-form`: name, geofence radius, a `map_picker` (`#location-map`) with coordinates (`#location-map-coordinates`) echoed in `#location-pending-point`; Add (disabled until a point exists) / Remove; a single location with a blank name becomes "Primary Site". `#attachment-form`: one file via presigned POST plus a **required** description. Delete removes the operation. **Copy** (`#copy-operation-<id>` on the index, `#copy-operation` on the edit page) opens the new-operation form at `/:id/copy` prefilled with the source's title, description, and visibility — never its window (`#copy-source-note`; the source's locations and attachments are listed in `#copy-locations` / `#copy-attachments`). Saving calls `Operations.copy_operation/3`, which in one transaction creates the operation, copies every location (name, point, radius, notes, position), and copies every attachment to a **fresh object key** via `Storage.copy_object/2` (recorded as uploaded by the copier), then redirects to the copy's edit page.
- **Member** (`/app/operations`): every visibility. Detail (`/app/operations/:id`): header with the time window, description, locations listed in text and drawn on a `static_map` with radius circles, attachments (description + filename, Download via a short-lived presigned URL; an id that matches nothing answers "no longer available" instead of crashing), an RSVP section (§7.28: counts in `#rsvp-counts` / `#rsvp-count-{yes,maybe,no}`; `#rsvp-form` with a response select and optional note, "Save RSVP" / "Update RSVP" (`#rsvp-submit`), flash "RSVP saved", the member's current answer in `#rsvp-status`; the form is replaced by `#rsvp-closed` once the operation has ended; the list in `#rsvps` / `#rsvp-<id>` shows name, call sign, response, and note, with `#rsvps-empty` when there are none), attendance list with source, and "Mark my attendance" (`source = manual`, flash "Attendance recorded"). RSVPs and attendance are independent: attendance is recorded for whoever shows up.
...
A private Fly Tigris bucket holds all uploads (asset images, course evidence, certification task books and certificates, operation attachments, documents). `McEmcomm.Storage` dispatches to a `Storage.Client` behaviour with four callbacks — `presign_upload/2`, `presign_download_url/1`, `delete_object/1`, `copy_object/2` — implemented by `Storage.S3` over `ReqS3`. LiveView `allow_upload` MUST use `external:` with a presigned POST form from `ReqS3.presign_form/1` whose policy carries a `content-length-range` cap of 8,000,000 bytes (mirroring LiveView's default, because `external:` uploads bypass the server) and a one-hour expiry; the browser posts the file directly with the `S3` uploader. Viewing MUST use short-lived URLs from `ReqS3.presign_url/1` (300 s); deletes presign a 60 s DELETE; copies are a signed server-side `PUT` on the destination key with `x-amz-copy-source` (used when copying an operation's attachments, §9.12), so the two operations never share an object. Keys are `<prefix>/<uuid>.<ext>`. Standard env vars `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `AWS_ENDPOINT_URL_S3` drive ReqS3 and `BUCKET_NAME` names the bucket; `AWS_ENDPOINT_URL_S3` is mirrored into `:storage_url` only so the CSP can name the bucket origin. Tests MUST stub the client (`McEmcomm.StorageMock`, Mox). Local development uses adobe/s3mock (which validates no signatures and ignores policy conditions — CONTRIBUTING.md § Setup).
...
**Starting.** Any approved member starts a net from the console with a name (blank → the start date), a required single-word APRS keyword unique among active nets (case-insensitive), and an optional operation. A net may only be assigned to an operation whose `starts_at`..`ends_at` window contains the present (`Operations.list_operations(active_at: now)`; `Net.start_session/2` and `Net.assign_operation/2` reject any other id with `operation_id: "must be in progress"`, so the MCP tools obey the same rule). The starter is stamped as `started_by_member_id`, becomes the initial net control operator, and — if they have a call sign — is logged as the first check-in with their QTH snapshotted. `{:nets_changed, :session_started}` is broadcast.
...
Outbound mail goes through Swoosh's Resend adapter from `{"Monroe County ARES/RACES", MAIL_FROM}` (local mailbox adapter in dev, test adapter in test). Account mail: confirmation/login instructions (magic link) and email-change instructions. A delivery failure MUST NOT crash the LiveView that requested it: registration still creates the account and tells the member to request a login link later, the email-change form reports that the message could not be sent, and the magic-link form keeps its neutral flash (an error there would reveal whether the address is registered); every failure is logged with the user id, never the address. Membership mail: when a new account registers, `Members.notify_new_member_registered/1` MUST email the users of approved holders of every `notify_on_new_member` position — deduplicated, one message per recipient so addresses are not shared — with subject "New member awaiting approval: <name>", the member's name, call sign (or "none given"), account email, a note that the address is not yet confirmed, a link to `/admin/members`, and a line explaining why the recipient received it. The notice fires at registration rather than first confirmation so leadership hears about a signup even when the confirmation email never reaches the member. Delivery runs under `McEmcomm.TaskSupervisor` so a mail outage can never fail the registration; confirmation and later logins send nothing. When a member transitions into `approved` (§4), `MemberNotifier.deliver_membership_approved/2` MUST email the member with subject "Your Monroe County ARES/RACES membership is approved", greeting them by name and linking to `/users/log-in`; it too runs under the task supervisor after the transaction commits, so a mail outage can never fail an approval. Inbound: `POST /webhooks/resend` (§24) records the delivery and hands the event to `Inbound.handle_event/1`, a no-op extension point.
...
**Tool catalog (`McEmcomm.MCP.Tools.Registry`, 34 tools).** Names are `snake_case`, ≤ 30 characters; every tool has a `title`, `description`, `inputSchema`, `outputSchema`, and accurate `readOnlyHint`/`destructiveHint`/`idempotentHint`; reads and writes are separate tools; list tools paginate with an opaque cursor (50 per page); ids returned by list tools are the ids other tools take; enums constrain statuses, visibilities, license classes, catalog kinds, RSVP responses, and attendance sources. JSON Schema validation is the hand-rolled subset in `McEmcomm.MCP.Schema` (`type`, `anyOf`, `enum`, `const`, `required`, `properties`, `additionalProperties: false`, `items`, `minimum`, `maximum`, `minLength`, `maxLength`, `minItems`, `maxItems`); nullable fields are expressed as `anyOf` single-type branches rather than `type: [..., "null"]`, which some MCP clients mis-read (the Inspector's `--strict` portability check passes clean); if full Draft 2020-12 validation is ever needed the single approved dependency is `jsv` (MIT), never `ex_json_schema`.
...
| `list_ops`, `get_op`, `list_op_rsvps`, `list_op_attendance` (read) | operations | `Operations.list_operations/1`, `get_operation/1`, `list_rsvps/1`, `list_attendance/1` |
| `rsvp_op` (idempotent) | operations · approved profile | `Operations.rsvp/3` (yes, maybe, no, optional note; refused once the operation has ended) |
